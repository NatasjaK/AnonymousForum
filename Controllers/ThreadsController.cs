...
namespace AnonymousForum.Controllers
{
    public class ThreadsController : Controller
    {
        ...
        // GET: Threads/TopicThreads/5
        public async Task<IActionResult> TopicThreads(int id)
        {
            var threadsTopic = await _context.Threads
        ...
        public async Task<IActionResult> Create([Bind("ThreadId,ThreadTitle,ThreadDescription,FkTopicId")] AnonymousForum.Models.Thread thread)
        {
            if (ModelState.IsValid)
            {
                ...
                // Redirect to the TopicThreads view, passing the FkTopicId to show threads for the specific topic
                return RedirectToAction("TopicThreads", new { id = thread.FkTopicId });
            }
